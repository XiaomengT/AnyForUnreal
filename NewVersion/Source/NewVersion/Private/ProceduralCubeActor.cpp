...
{
	mesh = CreateDefaultSubobject<UPersonalMeshComponent>(TEXT("PersonalCube"));
	...
	// Generate a cube
	TArray<FPersonalMeshTriangle> triangles;
	GenerateCube(100.f, triangles);
	mesh->SetPersonalMeshTriangles(triangles);

	RootComponent = mesh;

}

void AProceduralCubeActor::GenerateCube(const float& InSize, TArray<FPersonalMeshTriangle>& OutTriangles)
{
	...
	FPersonalMeshVertex v0;
	FPersonalMeshVertex v1;
	FPersonalMeshVertex v2;
	FPersonalMeshVertex v3;
	v0.U = 0.f; v0.V = 0.f;
	v1.U = 0.f; v1.V = 1.f;
	v2.U = 1.f; v2.V = 1.f;
	v3.U = 1.f; v3.V = 0.f;

	FPersonalMeshTriangle t1;
	FPersonalMeshTriangle t2;
